...
**README rewritten.** The previous version dated to 2026-08-17/18, written before the hardware had even arrived — its own Setup section read "Not yet documented," and its status line said "early." Replaced with the actual current state: real OAuth-backed calendar (not aspirational), live weather, calendar colour-coding, the colour picker, and `npm run setup` as the documented setup path. Left a clearly marked, commented-out image placeholder rather than a broken link — `docs/first-light.jpg` exists (the actual first frame push, 2026-08-17) but is a calibration test pattern (a colour-bar/registration-mark test image), not the product, and using it as the README's hero shot would misrepresent what the panel actually shows day to day. Needs a real photo from Ricky; `docs/**/*.jpg` is already exempted from `.gitignore` for this. README's License section corrected to not claim "MIT" outright, since no `LICENSE` file exists yet — that lands bundled with the public-repo flip, per the roadmap's explicit last-step ordering, not before it.
